...
async def rag_tool(request: RAGRequest):
    print(
        f"[LOG TOOL RAG] Вызов RAG с query: '{request.query}', device_type: '{request.device_type}'"
    )
    ...
    try:
        ...
        print(
            f"[LOG TOOL RAG] Неизвестный тип устройства: {request.device_type}. Использую 'cisco_ios' по умолчанию."
        )
...
async def execute_tool(request: ExecuteRequest):
    print(
        f"[LOG TOOL EXECUTE] Вызов EXECUTE с командами: {request.commands}, на устройстве {request.device_ip}, тип: {request.device_type}"
    )
    ...
    try:
        ...
        print(
            f"[LOG TOOL EXECUTE] Неизвестный тип устройства: {request.device_type}. Использую 'cisco_ios' по умолчанию."
        )
        device_type_enum = EquipmentType.CISCO_IOS
    ...
    try:
        ...
        output = await execute_commands_ssh(
            request.commands,
            request.device_ip,
            request.username,
            request.password,
...
async def chat_endpoint(request: ChatRequest):
    ...
    needs_config_action = any(
        keyword in user_query_lower for keyword in CONFIG_KEYWORDS
    )  # <-- Теперь проверяет и "показать"
    print(
        f"[LOG SERVER] Нужна ли настройка/просмотр (needs_config_action): {needs_config_action}, проверено по ключевым словам: {CONFIG_KEYWORDS}"
    )

    if needs_config_action:
        print(
            f"[LOG SERVER] Обнаружено действие, связанное с оборудованием (настройка или просмотр)."
        )
        if not request.device_type:
            print("[LOG SERVER] device_type отсутствует.")
            return {
                "response": "Неизвестен тип сетевого устройства. Пожалуйста, укажите тип (например, Cisco IOS, Juniper JunOS)."
            }

        print(
            f"[LOG SERVER] Выполняю поиск команд для запроса: '{user_query}' на устройстве типа {request.device_type}"
        )
        ...
        if not rag_results["results"]:
            ...
            return {
                "response": f"Не найдено подходящих команд в документации для запроса: '{user_query}' на устройстве типа {request.device_type}."
            }
        ...
        if "hostname" in user_query_lower or "имя хоста" in user_query_lower:
            ...
            "show" in user_query_lower
            or "показать" in user_query_lower
            or "display" in user_query_lower
        ):
            # Попробуем определить конкретную команду просмотра
            if (
                "running-config" in user_query_lower
                or "текущую конфигурацию" in user_query_lower
            ):
                primary_cmd_keyword = "show running-config"
            elif (
                "startup-config" in user_query_lower
                or "загрузочную конфигурацию" in user_query_lower
            ):
                primary_cmd_keyword = "show startup-config"
            elif "version" in user_query_lower or "версию" in user_query_lower:
                primary_cmd_keyword = "show version"
            elif "interface" in user_query_lower or "интерфейс" in user_query_lower:
                primary_cmd_keyword = "show interface"
            # ... можно добавить другие

        filtered_commands = []
        if primary_cmd_keyword:
            # Попробуем найти строки, содержащие ключевое слово
            # Используем более гибкий поиск, например, начинается с ключевого слова
            filtered_commands = [
                cmd_str
                for cmd_str in rag_results_list
                if primary_cmd_keyword.split()[0] in cmd_str.lower()
            ]  # Ищем первое слово ключа (например, "show")
            if filtered_commands:
                print(
                    f"[LOG SERVER] Найдены команды, соответствующие ключевому слову '{primary_cmd_keyword}': {filtered_commands}"
                )
                rag_results_list = filtered_commands
            else:
                print(
                    f"[LOG SERVER] Команды, соответствующие ключевому слову '{primary_cmd_keyword}', не найдены. Используем все результаты RAG."
                )
        ...
        if not suggested_commands:
            print(
                "[LOG SERVER] Команды найдены в документации, но не извлечены/обработаны."
            )
            return {
                "response": f"Найдены документы, но команды не извлечены или требуют уточнения. Проверьте формат файла документации для {request.device_type}."
            }

        # --- Валидация команд (может быть адаптирована для команд просмотра) ---
        ...
        validation_result = ConfigurationValidator.validate_commands(
            suggested_commands, device_type_enum
        )
        # Для команд просмотра можно игнорировать предупреждения или обрабатывать отдельно
        warning_context = ""
        if not validation_result["is_safe"]:
            warnings_str = "\n".join(validation_result["warnings"])
            print(
                f"[LOG SERVER] Обнаружены потенциально опасные команды: {warnings_str}"
            )
            # Для команд просмотра это может быть ложное срабатывание, но на всякий случай:
            # warning_context = f"\nПРЕДУПРЕЖДЕНИЕ: Были обнаружены потенциально опасные команды:\n{warnings_str}\nПожалуйста, убедитесь, что вы хотите выполнить их."
            # Или игнорируем для 'show'
            if any(cmd.startswith(("show", "display")) for cmd in suggested_commands):
                print("[LOG SERVER] Команда просмотра, предупреждения игнорируются.")
            else:
                warning_context = f"\nПРЕДУПРЕЖДЕНИЕ: Были обнаружены потенциально опасные команды:\n{warnings_str}\nПожалуйста, убедитесь, что вы хотите выполнить их."
        ...
        if exec_result.get("error"):
            ...
            return {
                "response": f"Ошибка при выполнения команд на устройстве: {exec_result['output']}"
            }

        # Для команд просмотра можно формировать более простой контекст
        context_for_model = f"""
        Пользователь хотел посмотреть информацию: "{user_query}".
        Были выполнены следующие команды: {suggested_commands}.
        Результат выполнения команд на устройстве: {exec_result['output']}.
        Пожалуйста, сформулируй ответ для пользователя, предоставив запрашиваемую информацию или пояснив результат.
        """
        ...
        # Если запрос не касается оборудования, просто прокинем его в модель
        print(f"[LOG SERVER] Обычный запрос, передаю в модель. Query: {user_query}")
        response = ollama.chat(
            model=model_name,
            messages=request.messages,  # <-- Передаём всю историю
            options={"temperature": 0.1},
        )
